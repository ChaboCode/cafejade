odoo.define('aspl_pos_kitchen_combo.ComboProductsWidget', function(require) {
    ...
    class ComboProductsWidget extends PosComponent {
        ...
        async willStart() {
          ...
          var combo_line_ids_new = await rpc.query({
                  model: 'product.combo',
                  method: 'get_combolines_by_sequence',
                  args: [0, combo_line_ids],
              });

          this.combo_line_ids_new = combo_line_ids_new
        }
        ...
        get categories() {
          //console.log("Ingresa aquiiiiiiiiiiiiwwwwwww ++++++++++++++++++++++++++++++++")
            var combo_line_ids = this.props.product.product_combo_ids;
            //console.log("Esto es var de combo_line_ids")
            //console.log(combo_line_ids)
            //console.log("Esto es combo_line_ids_new")
            //console.log(this.combo_line_ids_new)
            if (this.combo_line_ids_new){
              combo_line_ids = this.combo_line_ids_new
            }
        ...
        _switchCategory(event) {
            ...
            // ------------------------------- codigo para asignar categoria en env.pos --------------------------------
            this.env.pos.selectedComboCategoryId = this.state.selectedComboCategoryId
            this.render();
        }
        _createQuantityLine(){
            if(Object.keys(this.state.quantityLine).length == 0){
        ...
        _clickProduct(event){
            ...
            //console.log("Esto es event")
            //console.log(event)
            //console.log(event.detail.price_extra)
            //console.log(event.detail.quantityNew)
            ...
            if(this.state.useQuantityLine[this.state.selectedComboCategoryId] < this.maxQuantity){
                ...
                this.trigger('add-product', {
                    ...
                    extraPriceProduct: event.detail.price_extra,
                    quantityNew: event.detail.quantityNew,
                });
            }
        }
        _clickClear(event){
